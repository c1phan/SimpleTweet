...
function JSONToHTMLTable(elementToBind) {
    jsonData = firebase.database().getUsers();  
    ...
    for (var i = 0; i < jsonData.length; i++) {
        ...
        for (var j = 0; j < col.length; j++) {
            if(j == col.length - 1) {
              ...
              if(jsonData[i][col[2]] == 'Pending') {
                tabCell.innerHTML = "<button  class=\'users' type=\'button'>Verify</button>";
              } if(jsonData[i][col[2]] == 'Verified') {
                tabCell.innerHTML = "<button class=\'users' type=\'button' onclick=\'updateStatus(jsonData[i][col[1]]'>Void</button>";
              } 
            } else {
              var tabCell = tr.insertCell(-1);
